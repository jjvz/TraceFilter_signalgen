...
unsigned int TraceFilter::CalcFilters(const vector<double> *sig) {
    try{
        ...
        if(!isConverted_) ConvertToClockticks();
        CalcTriggerFilter();
        CalcBaseline();
        ...
        switch(errcode) {
        case(NO_TRIG) :
            cerr << "\nWe could not find a trigger in the trace." << endl;
            break;
        case(LATE_TRIG) :
...
//            break;
        case(BAD_FILTER_COEFF):
            cerr << "\nOne of the energy filter coefficients was nan." << endl;
            break;
        case(BAD_FILTER_LIMITS):
            cerr << "\nThe Energy filter was too long for the trace." << endl;
            break;
        }
        return(errcode);
    }
